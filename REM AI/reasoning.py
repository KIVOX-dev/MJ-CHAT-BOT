...
from config import GEMINI_API_KEY, DASHSCOPE_API_KEY
...
class ReasoningLayer:
    ...
    def generate_response(self, prompt: str) -> dict:
        ...
        system_instructions = (
            ...
            "- You ARE REM AI, an independent artificial intelligence.\n"
            "- NEVER claim to be developed by Microsoft, Google, OpenAI, or any other corporation.\n"
            "- If asked about your origin, state that you are REM AI, a high-performance intelligence engine designed for research and research assistance."
        )
        ...
        for iteration in range(3):
            ...
            for claim in corporate_claims:
                if claim in cleaned_text:
                    cleaned_text = cleaned_text.replace(claim, "As REM AI")
            
            # Remove redundant prefixes
            for prefix in ["Assistant Response:", "Assistant:", "AI:", "[REM AI]:"]:
    ...
    def _call_ai_engine(self, messages, fast_mode=False):
        ...
        In fast_mode (summaries), we prioritize speed (Cloud/Flash) over local privacy.
        """
        # 1. Cloud Preference for Speed (Summaries)
        if fast_mode:
            # Gemini 2.0 Flash is the speed king for context ingestion
            if self.gemini_client:
                try:
                    logging.info("Fast Path: Using Gemini Flash...")
                    gemini_messages = []
                    for m in messages:
        ...
        # 2. Local Intelligence (Primary for Data Privacy)
        if self.ollama_client:
            try:
                logging.info(f"Attempting Ollama chat ({self.preferred_local})...")
                # Add speed options for local inference
                options = {"num_predict": 300, "temperature": 0.4} if fast_mode else {}
                response = self.ollama_client.chat(
                    model=self.preferred_local, 
                    messages=messages, 
                    options=options
                )
                return {"text": response['message']['content'], "model": f"Ollama {self.preferred_local}"}
            except Exception as e:
                import requests
                try:
                     requests.get("http://127.0.0.1:11434/api/tags", timeout=10)
                except Exception:
                     logging.error("Ollama service DOWN.")
                     return {"text": "Local Intelligence Down. Please use Gemini/Qwen Cloud.", "model": "Offline"}

        # 3. Qwen/Cloud Fallback
        if dashscope and self.dashscope_key:
            try:
                logging.info("Attempting Qwen Cloud fallback...")
                res = dashscope.Generation.call(model=self.researcher_cloud, messages=messages, result_format='message')
                if res.status_code == 200:
                    return {"text": res.output.choices[0].message.content, "model": "Qwen 3.5 Cloud"}
            except Exception: pass

        # 4. Final Fail-Safe: Local Ensemble
        if self.learning_sys:
            try:
                last_user_msg = next((m["content"] for m in reversed(messages) if m["role"] == "user"), "")
                local_answer, confidence = self.learning_sys.predict(last_user_msg)
                if local_answer:
                    return {"text": local_answer, "model": "Local Ensemble"}
            except Exception: pass

        return {
            "text": "Critical: Engine response failure.", 
            "model": "None"
        }
